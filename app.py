...
# --- AUTO-INSTALAÇÃO DE DEPENDÊNCIAS (A MÁGICA) ---
def install(package):
    print(f">>> Instalando dependência que falta: {package}...")
    subprocess.check_call([sys.executable, "-m", "pip", "install", package])

required_packages = ['openpyxl', 'pandas', 'flask']
...
def init_db():
    with sqlite3.connect(DB_NAME) as conn:
        cursor = conn.cursor()
        cursor.execute('''CREATE TABLE IF NOT EXISTS clients (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, external_id TEXT UNIQUE, document TEXT, address TEXT, city TEXT, email TEXT, contract_num TEXT, contract_val REAL, contract_limit REAL, extra_val REAL, periodicity TEXT, created_at TEXT)''')
...
def closing():
    ...
    if request.method == 'POST' and 'file' in request.files:
        ...
        if file.filename:
            try:
                # 1. Tenta ler como EXCEL (Forçando openpyxl)
                try:
                    file.seek(0)
                    df = pd.read_excel(file, engine='openpyxl')
                except Exception as e_excel:
                    print(f"Erro Excel: {e_excel} - Tentando CSV...")
                    # 2. Se der erro, tenta ler como CSV (mesmo se chamar .xlsx)
                    file.seek(0)
                    try: df = pd.read_csv(file, encoding='latin1', sep=';', on_bad_lines='skip')
                    except:
                        file.seek(0)
